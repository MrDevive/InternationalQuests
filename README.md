...
**Ответ:** InternationalLevel опционален. Без него плагин не будет выдавать награды опытом и уровнями, но вся остальная функциональность (предметы, деньги, команды) будет работать.
...
**Ответ:** Да, плагин поддерживает два типа хранения данных: `flatfile` (файлы) и `mysql` (база данных). Выберите нужный вариант в `config.yml`.
